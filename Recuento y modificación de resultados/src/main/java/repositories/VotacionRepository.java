...
public interface VotacionRepository extends JpaRepository<Votacion, Integer> {
	...
	@Query("select count(r) from Respuesta r where r.voto.votacion.id = ?1 and r.pregunta = ?2")
	int numeroRespuestasPorPreguntaYVotacion(int votacionId, String pregunta);
	
		// 2. Cuantos respondieron con Y a la pregunta X
	@Query("select count(r) from Respuesta r where r.voto.votacion.id = ?1 and r.pregunta = ?2 and r.opcion = ?3")
	...
	@Query("select 1.0*count(r)/(select count(r1) from Respuesta r1 where r1.voto.votacion.id = ?1 and r1.pregunta = ?2) from Respuesta r " +
	...
	@Query("select 1.0*count(r)/(select count(r1) from Respuesta r1 where r1.voto.votacion.id = ?1 and r1.pregunta = ?2 and r1.voto.cp = ?4) from Respuesta r " +
				"where r.voto.votacion.id = ?1 and r.pregunta = ?2 and r.opcion = ?3 and r.voto.cp = ?4")
	...
	@Query("select count(r.opcion), r.opcion from Respuesta r where r.voto.votacion.id = ?1 and r.pregunta = ?2 group by r.opcion")
	...
	@Query("select count(r.opcion), r.opcion from Respuesta r where r.voto.votacion.id = ?1 and r.pregunta = ?2 and r.voto.cp = ?3 group by r.opcion")
